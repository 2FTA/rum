...
type DeleteRuleControlProps = {
  title: string;
  onConfirm: () => void;
};

export function DeleteRuleControl({
  title,
  onConfirm,
}: DeleteRuleControlProps) {
  ...
  return (
    <div ref={rootRef} className="relative shrink-0">
      <button
        ...
        data-open={open}
        aria-label={`Удалить правило «${title}»`}
        onClick={(event) => {
      ...
      {open ? (
        ...
        >
          ...
          <div className="mt-3 flex justify-end gap-2">
            <button
              type="button"
              className="rounded-notion px-2 py-1 text-xs text-notion-muted hover:bg-notion-hover"
            ...
            <button
              type="button"
              className="rounded-notion px-2 py-1 text-xs text-red-600 hover:bg-red-50"
              onClick={() => {
                onConfirm();
                setOpen(false);
              }}
